...
export async function ContactBranchGrid() {
      ...
      {contactBranchIds.map((id, i) => (
        <FadeIn key={id} delay={i * 0.06}>
          <article className="flex h-full min-h-0 flex-col rounded-2xl border border-brand-border bg-brand-surface p-6 shadow-card sm:p-8">
            <h2 className="font-display text-xl text-brand-frost sm:text-2xl lg:text-3xl">{t(`branches.${id}.name`)}</h2>
            <dl className="mt-6 space-y-4 text-sm">
              <div>
                <dt className="text-xs font-bold uppercase tracking-widest text-brand-muted">{t("labels.phone")}</dt>
                <dd className="mt-1">
                  <a
              ...
              <div>
                <dt className="text-xs font-bold uppercase tracking-widest text-brand-muted">{t("labels.email")}</dt>
